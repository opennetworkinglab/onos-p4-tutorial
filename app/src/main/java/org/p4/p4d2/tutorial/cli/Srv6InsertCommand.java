...
public class Srv6InsertCommand extends AbstractShellCommand {
    ...
    @Argument(index = 1, name = "dest", description = "Destination IPv6 Address",
            required = true, multiValued = false)
    String dest = null;

    @Argument(index = 2, name = "segments", description = "SRv6 Segments (space separated list)",
    ...
    protected void doExecute() {
        ...
        Ip6Address destIp = Ip6Address.valueOf(dest);
        ...
        if (sids.size() == 0) {
            ...
            return;
        } else if (!sids.get(sids.size() - 1).equals(destIp)) {
            // The last SID must be the target IP
            sids.add(destIp);
        }

        print("uri: %s, dest ip: %s, segments: %s, ",
                uri, destIp, sids.stream()
                                 .map(IpAddress::toString)
                                 .collect(Collectors.joining(",")));
        app.insertSrv6InsertRule(device.id(), destIp, 64, sids);
